...
export default function Card({ card, index }: { card: any; index: number }) {
  ...
  return (
    card && (
      ...
      >
        <Link
          href={`/portfolio/${card?.portfolio?.[0]?.slug}`}
          key={index}
          className="pointer-events-auto mb-5  w-full cursor-pointer"
        >
          {card?.portfolio?.[0]?.acf?.hero_image?.url && (
            <Image
              src={card?.portfolio?.[0]?.acf?.hero_image?.url}
          ...
          <div className="mt-2 flex items-center gap-2 sm:mt-sm sm:gap-4">
            {card?.portfolio?.[0]?.acf?.avatar?.url && (
              <Image
                src={card?.portfolio?.[0]?.acf?.avatar?.url}
                alt="Company Logo"
                width={60}
                height={60}
                quality={100}
            ...
            <div>
              <h2 className="mb-[3px] text-lg font-light sm:text-[26px]">
                {card?.portfolio?.[0]?.title?.rendered}
              </h2>
              <p className="text-xs font-light opacity-80 sm:text-sm">
                {card?.portfolio?.[0]?.acf?.sector}
              </p>
            </div>
          </div>
        </Link>
